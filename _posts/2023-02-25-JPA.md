...
En el repositorio se definen todas aquellas consultas que sean necesarias (lo puedes consultar tú en `org.springframework.data.repository.CrudRepository`. 
...
> * Si  empieza por `findCustomers` (con `s` final) devuelve una lista de `Customer`
