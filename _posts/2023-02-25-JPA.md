...
/**
 En una aplicación, el controlador es quien guía las peticiones del cliente al método apropiado.
 A continuación se muestra un ejemplo de controlador web
 @RestController
 ...
 public class NoteController {
     ...
     // Este controlador se activaría cuando en cliente visitase la url `notes/customer/{id}`,
     // donde `id` es una parte variable de la url. Por ejemplo, `notes/customer/5` devolvería
...
Vamos a crear una web a partir de la bd de `Customer`. Va a ser muy sencilla: mostrará los nombres de los clientes y, a continuación, la lista de los mensajes.
...
Para ejecutar el servidor Web hay que ejecutar el siguiente comando en la terminal `./mvnw spring-boot:run`
...
Aparecerá un mensaje de error, pero es normal, ya que no hemos creado todavía ninguna ruta en un controlador.

​	falta la captura para crear el controlador.
...
* `@GetMapping("/")` le indica que este método se encarga de lo ruta `/`, es decir, la raíz de la web
