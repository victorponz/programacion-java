...
La clase `Comparator` proporciona dos métodos esenciales para ordenar: `compare` y `thenCompare`. Al método de comparación se le pasa primero el valor que se va a comparar, y`thenComparing` se le pasa el siguiente valor que se va a comparar. El método `thenComparing` se puede utilizar muchas veces mediante métodos de encadenamiento, lo que permite utilizar valores prácticamente ilimitados para la comparación.
